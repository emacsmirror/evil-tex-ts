...
- =cic= (change inner command)
  ...
  *_Before:_* =\textbf{hel|lo}=

  *_Press_* =cic=

  *_After:_* =\textbf{|}=

  *_Before:_* =\tex|tbf{hello}=
  ...
  *_Before:_* =s|ome text \textbf{hello}=

  *_Press_* =cic=

  *_After:_* =some text \textbf{|}=
...
- =mtS= (change section type)
  ...
  *_Press_* =mtS=, select =subsection= from prompt
...
- =ysiwe= (surround word with environment)
  ...
  *_Press_* =ysiwe=, then type =equation=
...
- =cseea= (change environment to align)
  ...
  *_Press_* =cs= (change surround), =e= (old: environment), =e= (new: environment keymap), =a= (align)
